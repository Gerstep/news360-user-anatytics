  ...
  {  
    ...
    for(var i=0; i<metrics.length; ++i)
    {
      try
      {
        ...
        var metric = new ApplicationMetric( JSON.parse(response.getContentText()) )
        if(metric)
        {
...
function MetricsReporter(app, spreadSheet, metrics) {  
  ...
  {  
    ...
    metricsSheet.setColumnWidth(colorColumn, metricsSheet.getColumnWidth(colorColumn)*0.2);                
...
function drawChartFromShowMetrica()
{
      for(var index=column; index<(column+numColumns); ++index)
    {
      metricsSheet.autoResizeColumn(index);      
